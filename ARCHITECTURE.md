...
| Компонент | Роль |
|-----------|------|
| `Detector` | AST, сбор `findings`, детект `@skipCheck` |
| `ReadinessResolver` | `findings` + метка + deps → `ReadinessResult` (`actual`, `hasBlockers`, `skipCheck`) |
| `PresentationPlanBuilder` | план вывода и exit code (в т.ч. skip) |
| `AnalyseCommand` | CLI, оркестрация |
...
Detector.analyse(path)
    → AnalysisResult (теги, блокеры AST, сырые use-импорты, skipCheck)

UseDependencyChecker.check(result, path, projectRoot)
    → дополняет findings UseFinding при нарушении; пробрасывает skipCheck
