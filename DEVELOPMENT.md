...
1. **Тест** — описываем ожидаемое поведение из `READINESS_MODEL.md` (файл → уровень, блокер, метка).
2. **Код** — минимум, чтобы тест прошёл.
3. **Рефакторинг** — при зелёных тестах; Stan и Pint перед коммитом.

Пример: «файл с `$GLOBALS` → `Legacy`» — сначала тест, потом правило.
