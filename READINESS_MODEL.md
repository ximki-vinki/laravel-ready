...
Смысл: «у файла есть blockers, но временно не валить guard». CLI показывает findings и footer `Skipped: @skipCheck.`, exit `0`.

| Условие | Поведение |
|---------|-----------|
| есть readiness-метка + `@skipCheck` + blockers | exit `0`, findings видны, footer SkipCheck |
| есть метка + `@skipCheck`, blockers нет | обычный успех (skip не срабатывает) |
| `Untagged` / `MultiTag` + `@skipCheck` | **не** спасает — exit `1` (проблема конфигурации метки) |

Детект — `SkipCheckVisitor` → флаг `skipCheck` в `AnalysisResult` / `ReadinessResult`, не `TagFinding`. Политику применяет `PresentationPlanBuilder`.
