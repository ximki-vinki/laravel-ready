...
**Основной путь установки для пользователя:** `composer require-dev laravel-readiness/cli` → `vendor/bin/laravel-ready`.  
...
| Пакет | Назначение |
|-------|------------|
| `laravel-readiness/tool` | Исходники и разработка: `src/`, тесты, полный PHP-стек (`php ^8.5`, illuminate, symfony, …). |
| `laravel-readiness/cli` | Доставка пользователю: обёртка `bin/laravel-ready` + скачивание `.exe` из Release. Без `src/` и без runtime-зависимостей приложения. |

Пользователь **не** ставит `laravel-readiness/tool` — только `laravel-readiness/cli`.
...
| # | Шаг | Статус |
|---|-----|--------|
| 1 | `laravel-readiness/tool`: `composer.json` как пакет разработки (`type: library`, `src/`, тесты) | готово |
| 2 | GitHub Actions: сборка `laravel-ready.exe` по тегу → GitHub Release | готово |
| 3 | Пакет `laravel-readiness/cli`: тонкий dist + плагин скачивания exe + обёртка `bin/laravel-ready` | готово |
| 4 | Packagist: оба пакета из одного репо, общие теги `v*` | — |
| 5 | Проверка в другом проекте: `composer require-dev laravel-readiness/cli` + запуск | — |
...
### Шаг 3: пакет `laravel-readiness/cli`
...
```
laravel-ready/
  composer.json              ← laravel-readiness/tool (разработка)
  src/ …
  packages/cli/
    composer.json            ← laravel-readiness/cli (пользователи)
...
| Элемент | Что делает |
|---------|------------|
| `name: laravel-readiness/cli` | Имя на Packagist для `composer require-dev`. |
...
- `laravel-readiness/tool` — пакет разработки (исходники, CI, Box).
- `laravel-readiness/cli` — тонкий пакет доставки (без runtime-зависимостей приложения).
...
- В чужом проекте: `composer require-dev laravel-readiness/cli` → `vendor/bin/laravel-ready` работает на Windows.
...
- `laravel-readiness/tool` по-прежнему используется для разработки в этом репозитории, не как основной пакет для потребителей.
...
- `composer global require laravel-readiness/cli` как альтернатива project-local установке.
...
- Можно показать коллеге: одна команда `composer require-dev laravel-readiness/cli` + готовый пример хука.
...
- Platform-пакеты (`laravel-readiness/cli-windows`, `-linux`, `-darwin`) или единый `cli` с выбором артефакта по ОС.
