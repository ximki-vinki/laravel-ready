...
it('allows vendor import', function (): void {
    ...
    $rule = new DenyAppImportByLevelRule(appLocator(), [ReadinessLevel::LaravelReady])->isDenied($import);
...
it('denies laravel-ready app import when only adapter is allowed', function (): void {
    $import = new UseImportFinding('App\Domain\TaggedService', 5);
    $rule = new DenyAppImportByLevelRule(appLocator(), [ReadinessLevel::LaravelAdapter])->isDenied($import);
